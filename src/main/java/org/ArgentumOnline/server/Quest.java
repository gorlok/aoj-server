    ...
    public void hacerQuest(Client cliente, Npc npc) {
        try {
            ...
            if (cliente.getQuest().m_enQuest) {
                cliente.enviarHabla(COLOR_BLANCO, "Debes terminar primero la quest que estás realizando para empezar otra!", npc.getId());
                return;
            }
            ...
            switch (quest.Objetivo) {
                ...
                case OBJETIVO_ENCONTRAR_NPC:
                    ...
                    cliente.enviarHabla(COLOR_BLANCO, "Debes encontrar a mi amigo npc para recibir tu recompensa! Pistas: Se puede encontrar en lugares característicos del juego, pero apresúrate! Si alguien que está haciendo este mismo tipo de quest, y lo encuentra antes, puedes perderlo. En tal caso deberás volver y hacerme clic, y poner /MERINDO", npc.getId());
                    //Npc amigoNpc = 
                ...
                case OBJETIVO_MATAR_UN_NPC:
                    ...
                    cliente.enviarHabla(COLOR_BLANCO, "Debes matar al npc que se encuentra en las coordenadas " + this.Pista[azar-1] + " para recibir tu recompensa! PD: Si no te apresuras, y lo mata otro usuario, perderás esta quest. En tal caso deberás volver y hacerme clic, y poner /MERINDO", npc.getId());
                    //Npc criaturaNpc = 
                    ...
                    if (cliente.getEstads().NPCsMuertos >= quest.NPCs) {
                        ...
                        if (quest.DaObj) {
                            if (cliente.getInv().agregarItem(quest.Obj, 1) < 1) {
                                Map mapa = this.server.getMapa(cliente.getPos().map);
        ...
        if (cliente.getQuest().m_enQuest) {
            ...
            if (npc.getNPCtype() == Npc.NPCTYPE_AMIGOQUEST) {
                ...
                if (quest.Objetivo == 3) {
                    ...
                    cliente.hablar(COLOR_BLANCO, "Felicitaciones, me has encontrado, ahora debes volver con mi compañero por tu recompensa!", npc.getId());
                } else {
    ...
    public void sendInfoQuest(Client cliente) {
        ...
        switch (quest.Objetivo) {
            ...
            case 2:
                ...
                break;
            case 3:
                cliente.hablar(COLOR_BLANCO, "Debes encontrar a mi amigo npc para recibir tu recompensa! Pistas: Se pude encontrar en lugares característicos del juego, pero apresúrate porque si otro que está haciendo este tipo de quest lo encuentra primero puedes perderlo, en tal caso deberás clikearme y poner /MERINDO", npc.getId());
                break;
            case 4:
                cliente.hablar(COLOR_BLANCO, "Debes matar al npc que se encuentra en las coordenadas " + this.Pista + " para recibir tu recompensa! PD: Si no te apresuras y lo mata otro usuario perderás esta quest, en tal caso deberás clikearme y poner /MERINDO", npc.getId());
                break;
        }
    }
